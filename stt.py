...
def stream_audio_to_text(on_transcript_callback, silence_timeout=3):
    """
    Stream audio from microphone, send transcripts to callback after silence.

    Args:
        on_transcript_callback (function): called with complete transcript after silence.
        silence_timeout (float): seconds of silence to consider end of speech.
    """
    # Initialize PyAudio and open microphone
    ...
    def silence_checker():
        ...
        while not stop_event.is_set():
            time.sleep(0.2)
            with buffer_lock:
                if transcript_buffer and (time.time() - last_update_time > silence_timeout):
            ...
            if text_to_send:
                ...
                on_transcript_callback(text_to_send)

    threading.Thread(target=silence_checker, daemon=True).start()

    # --- WebSocket Handlers ---
    def on_open(ws):
        """Start sending microphone audio in a separate thread."""
        def send_audio():
            while not stop_event.is_set():
                try:
        ...
        if data.get("type") == "Turn":
            ...
            if transcript:
                with buffer_lock:
                    ...
                    last_update_time = time.time()
                # Optional: print partial transcript
